...
struct ContentView: View {
    ...
    @State private var statusMessage = "Drag&drop your icon 1024x1024 here"
  ...
  private func processAndSave(image: NSImage, to directory: URL, mode: GenerationMode) {
        ...
        provider.loadItem(forTypeIdentifier: "public.image", options: nil) { data, error in
            DispatchQueue.main.async {
                if let data = data as? Data, let image = NSImage(data: data) {
                    ...
                    self.statusMessage = "Image loaded! Choose mode and tap 'Generate'"
                }
            }
        }
        return true
    ...
    // Идеальная функция ресайза без бага 2054x2054
